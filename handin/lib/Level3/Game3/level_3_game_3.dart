...
class Level3Game3 extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _Game3();
}

CollectionReference game3_3 = FirebaseFirestore.instance.collection('Game11');
...
enum directiondontshootThisEither { LEFT, RIGHT }
DateTime now = DateTime.now();

class _Game3 extends State<Level3Game3> {
  ...
  double dontshootfigureseithery = -0.2;
  double dontshootfigureseitherx = 0.8;
  double dontshootfigureseithery_1 = -0.2;
  double dontshootfigureseitherx_1 = 0.8;
  ...
  var figuresDirectionDontShootThisEither = directiondontshootThisEither.LEFT;
  ...
  void playGame() async {
    ...
    if (gameActive == false) {
      ...
      startDontshootFigureThisEither();
      gameActive = true;
  ...
  void startDontshootFigureThisEither() {
    double time = 0;
    double heights = 0;
    double velocity = 75;
    Timer.periodic(Duration(milliseconds: 20), (timer) {
      ...
      setState(() {
        dontshootfigureseithery_1 = heightCoordinate(heights);
      });
      time += 0.1;
      heights = -5 * time * time + velocity * time;
      if (dontshootfigureseitherx_1 < -1) {
        figuresDirectionDontShootThisEither =
            directiondontshootThisEither.RIGHT;
      } else if (dontshootfigureseitherx_1 > 1) {
        figuresDirectionDontShootThisEither = directiondontshootThisEither.LEFT;
      }
      if (figuresDirectionDontShootThisEither ==
          directiondontshootThisEither.LEFT) {
        setState(() {
          dontshootfigureseitherx_1 -= 0.02;
        });
      } else {
        setState(() {
          dontshootfigureseitherx_1 += 0.04;
        });
      }
      if (gameActive == false) {
        _showDialog();
  ...
  void shoot() async {
    ...
    if (middleofshot == false) {
      Timer.periodic(Duration(milliseconds: 20), (timer) {
        ...
        if (dontshootfigureseithery_1 >
                heightDontShootCoordinate_2(heightShot) &&
            (dontshootfigureseitherx_1 - shootx).abs() < 0.2) {
          gameActive = false;
          callDontshootFigure_2 = false;
          dontshootfigureseitherx = 2;
        }
      });
    }
  }
  ...
  double heightDontShootCoordinate_2(double height) {
    double totalHeight = MediaQuery.of(context).size.height * 3 / 4;
    double dontshootfigureseithery_1 = 1 - 2 * height / totalHeight;
    return dontshootfigureseithery_1;
  }
                  ...
                  children: [
                    Container(
                      ...
                      image: DecorationImage(
                        image: AssetImage("asset/bubbleTrouble_3.png"),
                        fit: BoxFit.fill,
                      ),
                    )),
                    Container(
                    ...
                    Container(
                      alignment: Alignment(
                          dontshootfigureseitherx_1, dontshootfigureseithery_1),
                      child: DontShootThisEither(),
                    ),
                    Container(
...
void playHelp() {
  assetsAudioPlayer.open(Audio("asset/audio/help3.mp3"));
}
